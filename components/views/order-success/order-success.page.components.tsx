...
import { convertOrderStatusToLib } from '@/utils/order'
		...
		<div className="space-y-6 pb-30">
			...
			<div className="bg-white rounded-lg border p-6 mb-6 shadow-sm print:shadow-none print:border-0 print:p-0">
...

					{
						...
						data.items.map((item, index) => (
							<div key={index} className="flex justify-between text-sm">
								...
								<span className="w-1/6 text-right">{item.amount}</span>
								<span className="w-1/4 text-right">{item.total_amount} FCFA</span>
							</div>
						))
					}
				</div>
				...
				<div className="mt-6 text-center text-sm text-gray-500">
					<p>Mode de
						paiement: {data.payment_method === 'on_delivery' ? 'Paiement à la livraison' : data.payment_method}</p>
					<p className="mt-1">Statut: <span
						className="font-medium capitalize">{convertOrderStatusToLib(data.order_status)}</span></p>
					<p className="mt-1">Nous vous contacterons bientôt pour confirmer votre commande.</p>
					<p className="mt-4 border-t pt-4">Vous êtes commerçant ? Vendez sur <a href="https://belou.store"
																																								 target="_blank"
																																								 rel="noopener noreferrer"
																																								 className="font-medium hover:underline"
																																								 style={{ color: '#fe4000' }}>Belou</a> et
						développez votre activité en ligne !</p>
				</div>
			</div>
...
OrderSuccessPageComponent.displayName = 'OrderSuccessPageComponent'